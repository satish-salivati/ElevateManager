...
import { Conversation, MeetingDetails } from '../types';
import { getCoachingPrompts } from '../services/geminiService';
...
const CoachingPhase: React.FC<CoachingPhaseProps> = ({ meetingDetails, onComplete }) => {
  ...
  useEffect(() => {
    const fetchPrompts = async () => {
      setIsLoading(true);
      const prompts = await getCoachingPrompts(meetingDetails);
    ...
    return (
      <Card>
        <div className="p-8 text-center flex flex-col items-center justify-center h-64">
          <Spinner />
          <p className="mt-4 text-slate-600">Generating coaching questions...</p>
        </div>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <div className="p-8 text-center bg-slate-50 border-b border-slate-200">
        <h2 className="text-2xl font-bold text-slate-900">Coaching Conversation</h2>
        <p className="mt-2 text-slate-600">
          Ask these open-ended questions to foster growth and development.
        </p>
      </div>
      <div className="p-8">
        {currentConvo && (
          <div>
            <label className="block text-sm font-medium text-slate-700">
              Coaching Question {currentIndex + 1} of {conversations.length}
            </label>
            ...
            <div className="relative">
              <textarea
                ...
                placeholder="Capture the employee's response here..."
        ...
        <div className="mt-8 pt-6 border-t border-slate-200 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            ...
            <Button onClick={handleNext} disabled={!canProceed}>
              {currentIndex === conversations.length - 1 ? 'Continue to Feedback' : 'Next Question'}
            </Button>
          </div>
        </div>
      </div>
