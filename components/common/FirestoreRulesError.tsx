...
import { doSignOut } from '../../services/firebaseService';
...
const FirestoreRulesError: React.FC<FirestoreRulesErrorProps> = ({ error, onRetry }) => {
  ...
  if (error.details) {
    const isProfileError = error.type === 'PROFILE_CREATION_FAILED';
    return (
      <div className="text-center py-12 px-4 border-2 border-dashed border-red-300 rounded-lg bg-red-50">
        ...
        <h3 className="mt-4 text-xl font-bold text-red-900">{isProfileError ? 'Account Setup Incomplete' : 'Admin Permissions Required'}</h3>
        ...
        <p className="mt-4 text-xs text-slate-500">
          {isProfileError 
            ? "After updating your rules in Firebase, log out and sign up again." 
            : "After updating your security rules in the Firebase Console, please retry."
          }
        </p>
        <div className="mt-6">
          {isProfileError ? (
            <Button onClick={doSignOut}>Logout & Try Again</Button>
          ) : (
